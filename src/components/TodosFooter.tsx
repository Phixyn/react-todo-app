interface TodosFooterProps {
  totalTasks: number;
  doneTasks: number;
}

export default function TodosFooter({
  totalTasks,
  doneTasks,
}: TodosFooterProps) {
  return (
    <div className="px-4 md:px-6 lg:px-8 h-12 text-sm md:text-base bg-gray-300 dark:bg-zinc-900 border-t border-gray-400 dark:border-zinc-700 flex flex-wrap items-center text-gray-600 dark:text-zinc-400">
      <p className="flex-1 order-1" data-testid="total-tasks-count">
        {totalTasks} {totalTasks === 1 ? "task" : "tasks"}
      </p>
      <p
        className="flex-1 order-2 text-center"
        data-testid="completed-tasks-count"
      >
        {doneTasks} complete
      </p>
      <p
        className="flex-1 order-last text-right"
        data-testid="open-tasks-count"
      >
        {totalTasks - doneTasks} open
      </p>
    </div>
  );
}
